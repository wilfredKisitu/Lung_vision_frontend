...
// Diagnosis Provider (Persistent)
final diagnosisProvider =
    StateNotifierProvider<DiagnosisNotifier, AsyncValue<List<Diagnosis>>>((ref) {
...
final fetchOnHomeNavigationProvider = FutureProvider<void>((ref) async {
...
class PredictionCount {
  ...
  PredictionCount({required this.low, required this.medium, required this.high});
}

// Provider to compute total predictions by risk
final totalPredictionsProvider = Provider<PredictionCount>((ref) {
  ...
  if (diagnosisState is AsyncData<List<Diagnosis>>) {
    ...
    int lowCount = diagnoses.where((d) => int.tryParse(d.prediction) == 0).length;
    int mediumCount = diagnoses.where((d) => int.tryParse(d.prediction) == 1).length;
    int highCount = diagnoses.where((d) => int.tryParse(d.prediction) == 2).length;
...
final latestPredictionProvider = Provider<String>((ref) {
  final diagnosisState = ref.watch(diagnosisProvider);
...
final symptomsForDiagnosisProvider = Provider.family<Map<String, List<Map<String, dynamic>>>, Diagnosis>((ref, diagnosis) {
  ...
  final sortedSymptoms = symptomsMap.entries.toList()
    ..sort((a, b) => b.value.compareTo(a.value));

  final topSymptoms = sortedSymptoms.take(5).map((entry) {
    return {
      "name": entry.key,
      "rank": entry.value,
    };
  ...
  final remainingSymptoms = sortedSymptoms.skip(5).map((entry) {
    return {
      "name": entry.key,
      "rank": entry.value,
    };
...
final notificationsProvider = Provider<List<Map<String, dynamic>>>((ref) {
  ...
  if (diagnosisState is AsyncData<List<Diagnosis>>) {
    ...
    final notifications = diagnoses.map((diagnosis) {
      return {
        "id": diagnosis.id,
